...
2. **blocker - `sync --profile` bypasses OpenPack graph resolution, dependency installation, graph locks, and combined planning.**
   ...
   Fix: route profile sync through the same target-grouped graph planner as normal `install`, grouping all roots per `(runtime,target,transport,adapter config)` and applying one combined plan per target.
...
9. **major - `agentwheel uninstall <package>` does not persist removal from the requested root set, so the next sync can reinstall it.**
   ...
   Scenario: workspace config contains roots A and B. `agentwheel uninstall A` removes A-owned runtime files and keeps shared dependencies required by B. A later plain `agentwheel install` reads the unchanged config, includes A again in the graph roots, and reinstalls A. That contradicts the Phase B uninstall model of removing a root from the requested set and recomputing ownership.
