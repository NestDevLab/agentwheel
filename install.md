...
3. Verify the CLI.
4. Install lifecycle guidance, opt-in proactive discovery, or both when requested.
...
For lifecycle management guidance in a project-local Codex setup:
...
For proactive semantic suggestions and read-only trials, install the separate discovery module:
...
Do not install it for users who want lifecycle management without proactive suggestions.
...
With the optional `agentwheel-discovery` module installed, an agent may proactively suggest up to three relevant
...
```bash
agentwheel search "describe the capability you need" --semantic --type skill --json --limit 10
...
- The requested lifecycle or discovery companion skill is installed.
